...
def show_test_page():
    ...
    # Testlarni faqat bir marta olish va aralashtirish
    if "test_data" not in st.session_state or st.session_state.get("current_field") != selected_field_name:
        ...
        # Har bir savol uchun boshlang‘ich qiymat
    ...
    for i, (test_id, topic_id, question, options, answer) in enumerate(tests):
        ...
        selected_option = st.radio(
            ...
            options_list,
            index=options_list.index(st.session_state[key]) if st.session_state[key] in options_list else 0,
            key=key
        )
        # Bunda Streamlit o‘zi avtomatik session_state[key] ni o‘zgartiradi
        st.markdown("---")

    if st.button("✅ Супоридан"):
        correct = 0
        ...
        # Keyingi testlar uchun yangilashni so‘rash
        del st.session_state.test_data

        
        results = get_all_results()  # [(user_id, topic_id, correct_count, total_count), ...]
        topics = get_all_topics()    # [(topic_id, topic_name, field_id, difficulty), ...]
        ...
        if y.nunique() > 1:
            ...
            # Foydalanuvchiga yangi mavzularni tavsiya qilish
            ...
            recommended_topics = candidate_topics[y_pred == 0]  # 0 — o‘rganishga tavsiya qilinadi

            if not recommended_topics.empty:
                st.markdown("### 🧠 Мавзӯъҳои тавсияшӯда барои омӯзиши шумо:")
                for name in recommended_topics["topic_name"].values[:5]:  # faqat 5 ta ko‘rsatamiz
                    st.markdown(f"🔹 **{name}**")
            else:
                st.info("Ҳамаи мавзӯъҳо аз тарафи шумо омӯхта нашудааст.")
        else:
        ...
        if not recommended_topics.empty:
                    ...
                    for _, row in recommended_topics.iterrows():
                        ...
                        topic_id = row["topic_id"]
                            ...
                            # Mavzuga tegishli resource linklarini olish
    ...
    if st.button("🏆 Рейтинг"):
        ...
        conn = sqlite3.connect("learning_platform.db")  # kerakli bazangiz yo‘lini yozing
        cursor = conn.cursor()
        ...
        for i, (username, score) in enumerate(top_users, 1):
            st.markdown(f"**{i}. {username}** — {score} балл")
        conn.close()
    if st.button("📈 Натиҷаҳои шумо"):
        user_id = st.session_state.user_id
        ...
        # Ma'lumotlarni olish
        cursor.execute("""
            SELECT t.name AS topic, r.correct_count, r.total_count, r.timestamp
            FROM results r
        ...
        if rows:
            ...
            df = pd.DataFrame(rows, columns=["topic", "correct", "total", "timestamp"])
            ...
            fig = px.line(
                ...
                y="score_percent",
                color="topic",
                title="Натиҷаҳои тест дар вақт",
                labels={"timestamp": "Рӯз", "score_percent": "Натиҷаи шумо (%)"},
                markers=True
            )
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Ҳоло шумо ягон натиҷа надоред.")
        conn.close()
